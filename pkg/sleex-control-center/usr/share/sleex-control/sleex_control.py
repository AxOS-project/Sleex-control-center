...
from ui.main_window import SleexControl
...
def main():
    ...
    logger.log(LogLevel.Info, "Starting Sleex Control Center")
...
def setup_environment_and_dirs(logger):
    ...
    try:
        temp_dir = os.path.join("/tmp", "sleex-control")
...
def launch_application(arg_parser, logger, txt):
    ...
    win = SleexControl(txt, arg_parser, logger)
    logger.log(LogLevel.Info, "Main window created successfully")

    setproctitle("sleex-control")
