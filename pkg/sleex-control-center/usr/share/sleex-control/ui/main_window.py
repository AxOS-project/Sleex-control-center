...
class SleexControl(Gtk.Window):

    def __init__(self, txt: Translation, arg_parser: ArgParse, logging: Logger) -> None:
        # Initialize cache directory
        self.cache_dir = os.path.expanduser("~/.cache/sleex-control")
        ...
        with self._init_lock:
            try:
                ...
                super().__init__(title="Sleex Control Center")
        ...
        try:
            ...
            if self.minimal_mode:
                ...
                if len(self.tab_pages) == 1:
                    ...
                    self.set_title(f"Sleex Control Center - {tab_name}")
            ...
            if (self.arg_parser.find_arg(("-V", "--volume")) or self.arg_parser.find_arg(("-v", ""))) and "Volume" in self.tab_pages:
                ...
                active_tab = "Volume"
                if self.minimal_mode:
                    self.set_title(f"Sleex Control Center - Volume")
                ...
                active_tab = "Wi-Fi"
                if self.minimal_mode:
                    self.set_title(f"Sleex Control Center - Wi-Fi")
            elif (
                ...
                if self.minimal_mode:
                    ...
                    self.set_title(f"Sleex Control Center - {translated_tab_name}")
            elif (
    ...
    def show_app_info_dialog(self):
        ...
        dialog = Gtk.Dialog(
            title="About Sleex Control Center",
            parent=self,
        ...
        app_name_label.set_markup("<span size='xx-large' weight='bold'>Sleex Control Center</span>")
    ...
    def on_destroy(self, window):
        ...
        if not hasattr(self, "logging") or not self.logging:
            self.logging = logging.getLogger("SleexControlCenter")
