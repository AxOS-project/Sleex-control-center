        ...
        try:
            ...
            if "Operation not permitted" in error_msg:
                error_display = (
                    ...
                    "- USBGuard requires your user to have access to it to avoid running Sleex Control Center with sudo.\n\n"
                )
...
#                "notify-send",
    ...
    def automate(self, widget):
        try:
            subprocess.run([
                "pkexec", "bash", "/usr/share/sleex-control/utils/usbguard-permissions.sh",
            ], check=True)
        except subprocess.CalledProcessError as e:
            if hasattr(self.logging, 'log_error'):
